...
IF "%IDE_BIN_DIR:~-8%" == "bin\win\" (
  echo.
  echo    You are trying to run ServiceFile from generic distribution
  echo Please first copy content of "%IDE_BIN_DIR%"
  echo into its parent directory ("%IDE_BIN_DIR:~0,-4%"^)
  echo and then run servicefile.bat from there
  echo.
  pause
  exit
)
...
:: Try (in order): ServiceFile_JDK, servicefile%BITS%.exe.jdk, ..\jre, JDK_HOME, JAVA_HOME.
:: ---------------------------------------------------------------------
SET JDK=

IF EXIST "%ServiceFile_JDK%" SET JDK=%ServiceFile_JDK%
IF NOT "%JDK%" == "" GOTO check

SET BITS=64
SET USER_JDK64_FILE=%USERPROFILE%\.ServiceFile1.0\config\servicefile%BITS%.exe.jdk
SET BITS=
SET USER_JDK_FILE=%USERPROFILE%\.ServiceFile1.0\config\servicefile%BITS%.exe.jdk
...
IF NOT EXIST "%JAVA_EXE%" (
  ECHO ERROR: cannot start ServiceFile.
  ECHO No JDK found. Please validate either ServiceFile_JDK, JDK_HOME or JAVA_HOME points to valid JDK installation.
  ECHO
  EXIT /B
)
...
:: ---------------------------------------------------------------------
IF NOT "%ServiceFile_PROPERTIES%" == "" SET IDE_PROPERTIES_PROPERTY="-Didea.properties.file=%ServiceFile_PROPERTIES%"

:: explicit
SET VM_OPTIONS_FILE=%ServiceFile_VM_OPTIONS%
...
IF NOT EXIST "%VM_OPTIONS_FILE%" (
  :: user-overridden
  SET VM_OPTIONS_FILE=%USERPROFILE%\.ServiceFile1.0\config\mps%BITS%.exe.vmoptions
)
IF NOT EXIST "%VM_OPTIONS_FILE%" (
  :: default, standard installation
  SET VM_OPTIONS_FILE=%IDE_BIN_DIR%\servicefile%BITS%.exe.vmoptions
)
IF NOT EXIST "%VM_OPTIONS_FILE%" (
  :: default, universal package
  SET VM_OPTIONS_FILE=%IDE_BIN_DIR%\win\servicefile%BITS%.exe.vmoptions
)
IF NOT EXIST "%VM_OPTIONS_FILE%" (
...
SET IDEA_PATHS_SELECTOR=ServiceFile1.0
